...
(defvar emacs-wiki-version "2.41-pre"
...
(defun emacs-wiki-directories-member (&optional directories)
  "Return non-nil if the current buffer is in
                           ...
                           (expand-file-name
                            (if (consp (car d))
                                (caar d) (car
                                          d)))))
                 ;; not an ignored file
                 (not (string-match emacs-wiki-file-ignore-regexp
                                    buffer-file-name)))
...
This makes the bad link face in the linking buffer go away."
  (let ((link (emacs-wiki-wiki-link-target link-name))
    ...
    (if (emacs-wiki-wiki-url-p link)
        (funcall emacs-wiki-browse-url-function link other-window)
      ...
      (let* ((base (emacs-wiki-wiki-base link-name))
        ...
        (if (null file)
	    (setq newbuf (if other-window (find-file-other-window base) (find-file base)))
	  (setq newbuf (if other-window (find-file-other-window file) (find-file file)))
          (when tag
            (goto-char (point-min))
            (re-search-forward (concat "^\\.?#" tag) nil t)))
        (when refresh-buffer
          (with-current-buffer newbuf
            (add-once-hook 'after-save-hook
                           `(lambda (&rest args)
                              (emacs-wiki-refresh-buffer
                               ,refresh-buffer
                               ; ,newbuf)
                               )) t t)))))))

(defun emacs-wiki-refresh-buffer (buffer &optional child)
  "When called interactively, refresh the current buffer by rebuilding the
file alist for the current project, and refontifying.

When called from a lisp script, an optional child argument will
add child to the file alist, saving having to rescan the entire
list of `emacs-wiki-directories'.

The child support is currently broken. Patches welcome."
  (interactive (list (current-buffer)))
  ;; it's possible when called non-interactively that the buffer to refresh
  ;; no longer exists. if that's the case, do nothing.
  (when (buffer-live-p buffer)
    (with-current-buffer buffer
      (emacs-wiki-file-alist)
      (font-lock-fontify-buffer))))

;;     (if (and child (buffer-live-p child))
;;      (with-current-buffer child
;;        ;; add the child buffer directly to the file list - no need
;;        ;; to update mtime
;;        (push (cons (emacs-wiki-page-name buffer-file-name)
;;                    buffer-file-name)
...
(defun emacs-wiki-follow-name-at-mouse (event &optional other-window)
  ...
  (save-excursion
    ...
    (if (emacs-wiki-link-at-point)
        (emacs-wiki-visit-link (match-string 0) nil other-window))))
...
(defun emacs-wiki-rename-link-at-point ()
  ...
  (interactive "*")
  (let (new-name old-name new-link)
    (if (emacs-wiki-link-at-point)
        (progn
          ...
          (emacs-wiki-refresh-buffer (current-buffer)))
      (error "There is no valid link at point"))))
...
bad-link face"
  (if (let ((base (emacs-wiki-wiki-base link-name)))
...
(defun emacs-wiki-highlight-link ()
  (if (eq ?\[ (char-after (match-beginning 0)))
      (if (and emacs-wiki-inline-images
               ;; make sure it's not a URL - we can't inline these
               (let ((c (aref (match-string 4) 0)))
                 (or (eq ?. c) (eq ?/ c)))
               (save-match-data
        ...
        (let* ((link (match-string-no-properties 4))
               (props (emacs-wiki-link-properties link
                          (emacs-wiki-link-face (match-string 4))))
               (invis-props (append props (emacs-wiki-link-properties link))))
          (if (match-string 6)
              (progn
...
(defun emacs-wiki-strip-tags (string)
    ...
    (let ((depth 1) (sub-open 0) (p (point)))
      ...
      (while contents
        ...
        (insert "<a href=\"#sec" (int-to-string index) "\">"
                (emacs-wiki-strip-tags (cdar contents))
                "</a>\n")
        (setq index (1+ index))
        (insert "</dt>\n")
            ...
            (if (null url)
              ...
              (if (save-match-data
                  ...
                  (if (string-equal url name)
                    ...
                    (concat "<a href=\"" url "\"><img src=\"" name "\"></a>"))
                (concat "<a href=\"" url "\">" name "</a>"))))
      (add-text-properties 0 (1- (length string)) '(rear-nonsticky (read-only) read-only
                                                                   t) string)
      string)))
...
(defsubst add-once-hook (hook func &optional append local)
  "Like add-hook, but for a function which should run once then remove itself
from the hook."
  (let ((fun (gensym)))
