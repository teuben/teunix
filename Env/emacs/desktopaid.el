...
;; $Id: desktopaid.el,v 1.1 2005/02/24 06:00:01 pteuben Exp $
...
; is in the Emacs load-path variable.  Include the following line in
; your .emacs file:
;
...
(defun dta-init-desktopaid ()
  ...
  (interactive)
  (dta-load-session (dta-get-config t)))
...
(defun dta-load-session (file-name)
  ...
  (interactive (list (dta-get-config t)))
  (save-excursion
...
(defun dta-load-frame-list (in)
  ...
  (let* ((toread (read in))
         ...
         (new-frames (- (length toread) (length (frame-list))))
         (selected nil))
    (while (> new-frames 0)
      (make-frame)
    ...
    (while toread
      (let* ((params (car toread))
        ...
        (setq selected (next-frame selected nil))
...
(defun dta-get-config (&optional must-match)
  ...
  (let ((insert-default-directory nil))
    (concat dta-cfg-dir (read-file-name "Select configuration: "
                                        dta-cfg-dir dta-default-cfg must-match))))
